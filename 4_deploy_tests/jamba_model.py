...
class JambaConfig(ConfigBase):
    ...
    def __post_init__(self):
        ...
        if self.attention_bias:
            raise ValueError('Only "False" attention_bias is supported for Jamba')
...
class JambaModel(nn.Module):
    def __init__(self, config: JambaConfig):
        ...
        self.embed_tokens = JambaEmbedding("vocab_size", config.hidden_size)
        self.layers = nn.ModuleList(
            [JambaDecoderLayer(config) for _ in range(config.num_hidden_layers)]
        )
